...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanZeroMaskAIns/"
    ...
    print(reRun)
    methodTags =algoDisp
    elapsedTimeAll, memLoadAll, periodAll, instructions, memStoreAll, fpVectorAll, fpScalarAll, branchAll,froAll = compareMethod(exeSpace, commonBasePath, resultPaths, csvTemplate,algosVec,nnzAValues, reRun)
    ...
    groupLine.DrawFigureYnormal(periodAll, froAll*100.0,
                                ...
                                figPath + "/"  + "nnz_err",
                                True)
    groupLine.DrawFigureYnormal(periodAll, (memInsAll)/instructions*100.0,
                                methodTags,
                                "nnz ratio", "propotion of mem access (%)", 0, 1,
                                figPath + "/"  + "nnz_mem",
                                True)
    groupLine.DrawFigureYnormal(periodAll, (branchAll)/instructions*100.0,
                                methodTags,
                                "nnz ratio", "propotion of branch (%)", 0, 1,
                                figPath + "/"  + "nnz_branch",
                                True)
    groupLine.DrawFigureYnormal(periodAll, (otherIns)/instructions*100.0,
                                methodTags,
                                "nnz ratio", "propotion of others (%)", 0, 1,
                                figPath + "/"  + "nnz_other",
                                True)
