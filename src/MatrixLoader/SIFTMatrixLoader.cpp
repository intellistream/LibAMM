...
void AMMBench::SIFTMatrixLoader::paraseConfig(INTELLI::ConfigMapPtr cfg) {
  ...
  if (aRow != 0 && aCol != 0 && bCol != 0) {
    if (aRow != bCol) {
      std::runtime_error(
          "in PCA task, B=A.t, so bCol==aRow should be hold all the time. Otherwise we can not get a symmetric real matrix for the rest of PCA task");
    }
    INTELLI_INFO(
        "aRow, aCol, bCol are specified in config.csv, so will make the default SIFT dataset [128x10000]*[10000x128] into the specified size ["
            + to_string(aRow) + "x" + to_string(aCol) + "]*[" + to_string(aCol) + "x" + to_string(bCol) + "]");
  } else { // use default SIFT dataset 128*10000, 10000*128
    INTELLI_INFO(
        "At least one of [aRow, aCol, bCol] is not specified in config.csv, so will use default SIFT dataset [128x10000]*[10000x128]");
  }
}
...
void AMMBench::SIFTMatrixLoader::generateAB() {
  ...
  // Step1. locate file
  std::string filename =
      "../../../../../../datasets/siftsmall_base.fvecs"; //benchmark execute path e.g. /home/heyuhao/AMMBench/build/benchmark/scripts/PCA/results/scansketchDimension_datasetSIFT/crs/100, dataset file path e.g. /home/heyuhao/AMMBench/build/benchmark/datasets/siftsmall_base.fvecs
  float *data = NULL;
  unsigned num, dim;
