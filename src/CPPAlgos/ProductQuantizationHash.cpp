...
int compute_hash_bucket(const std::vector<int>& split_indices, const std::vector<LibAMM::Tensor>& split_thresholds, const LibAMM::Tensor& x) {
    int i = 0;
    for (int t = 0; t < 4; t++) {
        int j_t = split_indices[t];
        LibAMM::Tensor v_t = split_thresholds[t];
...
LibAMM::Tensor LibAMM::ProductQuantizationHash::amm(LibAMM::Tensor A, LibAMM::Tensor B, uint64_t sketchSize) {
    ...
    LibAMM::Tensor split_indices_tensor = container.attr("split_indices").toTensor();
    ...
    LibAMM::Tensor v1 = container.attr("v1").toTensor();
    LibAMM::Tensor v2 = container.attr("v2").toTensor();
    LibAMM::Tensor v3 = container.attr("v3").toTensor();
    LibAMM::Tensor v4 = container.attr("v4").toTensor();
    std::vector<LibAMM::Tensor> split_thresholds = {v1, v2, v3, v4};
  ...
  for (int i = 0; i < A.size(0); ++i) {
    ...
    for (int c = 0; c < C; ++c) {
      ...
      a_encoded.push_back(LibAMM::tensor(closest_prototype_index).unsqueeze(0));
    }
