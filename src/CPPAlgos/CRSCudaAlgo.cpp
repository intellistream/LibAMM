...
namespace AMMBench {
    torch::Tensor AMMBench::CRSCudaAlgo::amm(torch::Tensor A, torch::Tensor B, uint64_t k) {
        A = A.t();
        ...
        torch::Tensor result = torch::matmul(A_sampled.to(torch::kCUDA), B_sampled.to(torch::kCUDA));
