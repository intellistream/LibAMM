...
class MaddnessMatmul(nn.Module):
    ...
    @torch.jit.export
    def _jit_forward(self, B):
        return self.apply_matmul_e2e(self.A, B)
    ...
    def apply_matmul_e2e(
        self, A, B, A_learn = None  # type: ignore
    ):
        if A_learn is None:
            self._learn_hash_buckets_and_prototypes(A)
        else:
...
def main():
    ...
    # A = torch.rand(10000, 512)
    # B = torch.rand(512, 100)
    A = np.random.rand(10000,512)
    B = np.random.rand(512,100)
    c = 16 # 16 regions at best.

    t = time.time()
    AB = torch.matmul(torch.tensor(A), torch.tensor(B))
    ...
    t = time.time()
    result = maddness._jit_forward(B)
    print("QUANT time: ", time.time() - t)
    print("QUANT error: ", torch.norm(AB - torch.tensor(result), p='fro'))

    # problem
    # Tried to set nonexistent attribute: A. Did you forget to initialize it in __init__()?:
        # File "/home/kailian/AMMBench/benchmark/torchscripts/Quantilization/Quantilization.py", line 336
        #   def _learn_hash_buckets_and_prototypes(self, A) -> None:
        #       self.A = A
