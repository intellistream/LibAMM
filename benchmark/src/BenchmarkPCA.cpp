...
void benchmarkPCA(std::string configName){
    ...
    // Step2. Test elapsedTime and energy on AMM in streaming and parallelism
    // 2.1 Run AMM
    ...
    pef.start();
    torch::Tensor C = br.parallelForward();
    ...
    // 3.1 Calculate errors
    INTELLI_INFO("3.1 evaluating the error, may takes some time");
    torch::Tensor realC = torch::matmul(A, B);
    // continue on PCA task excluding AMM part
