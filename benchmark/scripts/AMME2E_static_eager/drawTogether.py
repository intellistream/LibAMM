...
def runPeriod(exePath, srcA,srcB, algoTag, resultPath, configTemplate="config.csv",prefixTag="null"):
    ...
    try:
        ...
        print(f"Error {e}")
        os.system(f"cd {exePath} && sudo cp scripts/AMME2E_default_results/default.csv {resultPath}/{prefixTag}/default.csv")
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/AMME2E_static_eager/"
    ...
    groupBar2.DrawFigure(dataSetNames, np.log(lat95All), methodTags, "Datasets", "95% latency (ms)",
                         5, 15, figPath + "sec4_1_e2e_static_eager_latency_log", True)
    groupBar2.DrawFigure(dataSetNames, np.log(thrAll), methodTags, "Datasets", "elements/ms",
                         5, 15, figPath + "sec4_1_e2e_static_eager_throughput_log", True)
