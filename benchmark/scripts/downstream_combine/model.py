...
class MLP(nn.Module):
    ...
    def _create(self, hidden, layer, dropout=None, algo='mm', e=1):
        ...
        for i in range(layer):
            if i == 0:
                d['linear' + str(i)] = approx_Linear(784, hidden, sample_ratio=sample_ratio, minimal_k=minimal_k, sample_ratio_bwd=sample_ratio_bwd, minimal_k_bwd = minimal_k_bwd, sample_ratio_wu = sample_ratio_wu, minimal_k_wu = minimal_k_wu, algo=algo)
                d['relu' + str(i)] = nn.ReLU()
                if dropout:
                    d['dropout' + str(i)] = nn.Dropout(p=dropout)
            elif i == layer - 1:
                d['linear' + str(i)] = approx_Linear(hidden, 10, sample_ratio=sample_ratio, minimal_k=minimal_k, sample_ratio_bwd=sample_ratio_bwd, minimal_k_bwd = minimal_k_bwd, sample_ratio_wu = sample_ratio_wu, minimal_k_wu = minimal_k_wu, algo=algo)
            else:
                d['linear' + str(i)] = approx_Linear(hidden, hidden, sample_ratio=sample_ratio, minimal_k=minimal_k, sample_ratio_bwd=sample_ratio_bwd, minimal_k_bwd = minimal_k_bwd, sample_ratio_wu = sample_ratio_wu, minimal_k_wu = minimal_k_wu, algo=algo)
                d['relu' + str(i)] = nn.ReLU()
                if dropout:
                    d['dropout' + str(i)] = nn.Dropout(p=dropout)
        return d
