...
def compareMethod(exeSpace, commonPathBase, resultPaths, csvTemplate,algos,dataSetName,reRun=1):
    ...
    for i in range(len(algos)):
     ...
     #exit()
        if resultIsComplete:
            elapsedTime, cpuCycle, memStall, instructions, l1dStall, l2Stall, l3Stall,totalStall = readResultVector(dataSetName, resultPath)
...
def main():
    ...
    #exit()
    methodTags =algoDisp
    elapsedTimeAll, cpuCycleAll, periodAll, instructions, memStallAll, l1dStallAll, l2StallAll, l3StallAll,totalStallAll = compareMethod(exeSpace, commonBasePath, resultPaths, csvTemplate,algosVec,nnzAValues, reRun)
    ...
    groupLine.DrawFigureYnormal(periodAll, (memStallAll)/totalStallAll*100.0,
                                methodTags,
                                "nnz ratio", "propotion of TLB/DDR stall", 0, 1,
                                figPath + "/"  + "nnz_mem_stall",
                                True)
    groupLine.DrawFigureYnormal(periodAll, ipcAll,
                                methodTags,
                                "nnz ratio", "IPC", 0, 1,
                                figPath + "/"  + "nnz_IPC",
                                True)
    groupLine.DrawFigureYnormal(periodAll, (otherStallsAll)/totalStallAll*100.0,
                                methodTags,
                                "nnz ratio", "propotion of other stall", 0, 1,
                                figPath + "/"  + "nnz_OtherStall",
                                True)
    print(ipcAll)
