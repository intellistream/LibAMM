...
def main():
    ...
    groupLine.DrawFigure(periodAll, elapsedTimeAll,
                         methodTags,
                         "A's rank", "elapsed time (ms)", 0, 1,
                         figPath + "/" + scanTag + "_elapsedTime",
                         True)
    groupLine.DrawFigureYnormal(periodAll, cacheMissAll,
                                methodTags,
                                "A's Rank", "cacheMiss (%)", 0, 1,
                                figPath + "/" + scanTag + "_cacheMiss",
                                True)
    groupLine.DrawFigureYnormal(periodAll,
                                fro * 100.0,
                                methodTags,
                                "A's Rank", "normalized error %", 0, 100, figPath + "/" + scanTag +
                                "_froError",
                                True)
    groupLine.DrawFigureYnormal(periodAll,
                                eb * 100.0,
                                methodTags,
                                "A's Rank", "error bound ratio %", 0, 100, figPath + "/" + scanTag
                                + "_ebRatio",
                                True)
